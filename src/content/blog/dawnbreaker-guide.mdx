...
![](/src/assets/images/shipdepiction.png)
...
![](/src/assets/images/dawnbreaker-church.png)

And here's where this is located on the map:

![](/src/assets/images/dawnbreaker-church-map.png)
...
After that, we'll head on over to the miniboss at the Inn. You'll want to land behind it, not in the courtyard. Here's what that will look like:

![](/src/assets/images/dawnbreaker-inn.png)And the map as before:

![](/src/assets/images/dawnbreaker-inn-map.png)
...
![](/src/assets/images/dawnbreaker-house.jpg)At worst, you can just land on the back of the house, here, where there are just two small little worms. I would communicate this or ping your party if you plan on doing this, though.

![](/src/assets/images/dawnbreaker-house2.png)

And, as before, where this is on the map:

![](/src/assets/images/dawnbreaker-house-map.png)Once that's done, engage the boss wherever you desire. The common spots are on the hill leading from the house to the courtyard by the inn, or the courtyard itself, or right by the inn.

Here's where I recommend you fight him, by in-game view and map location.

![](/src/assets/images/dawnbreaker-boss-fightspot.png)![](/src/assets/images/dawnbreaker-boss-fightspot-map.png)
...
These are just cannon fodder, really. They'll melee the tank and do short, non-interruptible cast to apply **Darkblade** to themselves, empowering their autos to deal shadow damage and grant them stacking attack speed. You can dispel this, and should as needed. Not particularly threatening by themselves.

**Nightfall Shadowmage** - Night Bolt (Everyone), Empowering Shadows (Everyone)

**Night Bolt** is their spammable filler cast. It does a moderate amount of damage and should be kicked or stopped, but isn't the priority kick for this mob. **Empowering Shadows** is the cast you should kick (and it will recast if you use a stop such as a stun or knock-up) as it will apply a curse to its target that does a fair amount of damage and will slow them.

**Nightfall Ritualist** - Tormenting Ray (Healer, DPS), Stygian Seed (Healer, DPS)

The main lieutenant mob of this area, and correspondingly the greatest threat in whatever pack they are present. **Tormenting Ray** will pick one target and do a non-interruptible channel on them over several seconds. **Stygian Seed** will apply to a target and, after a few moments, explode, dealing damage to them and anyone around them.

**Sureki Webmage** - Web Bolt (Everyone), Bursting Cocoon (Healer, DPS)

This is a lieutenant mob, but it's not particularly threatening. **Web Bolt** is an interruptible cast that does a moderate amount of damage. **Bursting Cocoon** is your typical "get away from me" spread mechanic that goes on one person. Just pop personals on packs with these and you'll be fine.

## Speaker Shadowcrown

This is overall an easy boss, mechanically, but it can quickly go to heck if you stop paying attention for a few seconds.

Abilities:

**Shadow Bolt** - (Everyone)

The spammable filler the boss will cast. Not really a kick rotation kind of thing, but you should kick every one that you can.

**Burning Shadows** - (DPS, Healer)

Shadowcrown will debuff one player with a rather nasty dot that also slows them for a good amount. It is dispellable, and upon removal will give the entire party a healing absorb that also does increasing shadow damage.

**Collapsing Night** - (DPS, Healer)
...
**Dark Floes** does a large amount of pulsing shadow damage over a few seconds, and if there are ever two of these at the same time you should definitely consider using a personal or group wide DR. **Abyssal Rot** will debuff two targets with a shadow DoT that does a fair amount of damage. **Black Hail** will prefer ranged, and drop a series of swirlies at their feet that hurt quite a bit if you get hit.
...
These will cast **Tormenting Beam**, a high priority channel that does a large amount of a short duration. This is kickable, and should be kicked every time. It also can be stopped, but it will be recast when stopped. **Umbral Barrier** will buff them with an absorb that lasts for 6 seconds. If the absorb isn't broken or dispelled, they will gain permanent max health, essentially healing them.
...
You should tank this in an area with a clear lane to aim the Orb down. At a similar frequency to the other minibosses, they'll do a decently long cast, targeting a player with a slowly growing orb. At the end of the cast, the orb will roll in that player's direction, traveling until it despawns naturally or encounters terrain. It can roll downwards or roll upwards, just not into something. This does falloff damage, so the further you aim it, the better. If this travels the minimum distance (aka barely goes anywhere), there is a fairly high chance you will straight up wipe. It cannot be stressed enough that you must master this mechanic, as it, like all the other miniboss mechanics are present on the second boss. Place a marker if you must, or ping every time this comes out.
...
This obviously is a rather easy lineup. If this happens, it will be Orb->Slam->Adds->Orb->Slam. Simply do everything as normal, and you should avoid using any personals during this.

**Orb into Decay combo:**

This is easily the most dangerous, and as you can imagine requires heavy personal/groupwide DR usage. This has two variants, from what I've seen.
...
on higher keys (the 15+ range), I actually recommend NOT letting the orb travel the full distance in the event of the second variant, as this will make its detonation occur with one of the higher damage ticks.

What you should do in the case of an immediate **Shadowy Decay** is aim it into terrain that is far enough away that it will explode in the outer rings of the "dropoff range", so that it explodes earlier but still doesn't kill you.

If you get the first variant, you can still safely do a full range orb.

**No Adds** combo:

The boss will simply rotate through its full ability cycle without summoning adds. This means you still get two **Shadowy Decays**, but it will still occur far enough apart that you shouldn't be pressured too greatly.
...
This miniboss will cast **Night Bolt** once or twice in between other casts, and you should soft priority kick it, unless a more important mob is up. I believe the **User Reinforcement** cast is percentage based, at 75%, 50%, and 25%, but it's very common for CDs to be sent into this, so it commonly pushes in a strange way as the **Tormenting Eruption** cast is quite long.
...
You should deal with these as you did throughout the dungeon and group them onto the Architect for cleave, still focusing on the Architect. When it reaches 1%, it will briefly go immune and summon the final boss, Rasha'nan.
...
![](/src/assets/images/dawnbreaker-route-pic.png)
